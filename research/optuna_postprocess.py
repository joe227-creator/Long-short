"""Validation-only Optuna selection for a committed portfolio parameter."""
...
def _set_parameter(spec, value):
    name = spec["parameter"]
    if name not in {"VOL_GATE_THRESHOLD", "VOL_GATE_STRENGTH", "CASH_BIAS"}:
        raise ValueError(f"Unsupported Optuna parameter: {name}")
    setattr(_train, name, float(value))


def _evaluate(value, spec, signals, targets, vol_forecast, dates, frequency, metric_fn):
    _set_parameter(spec, value)
...
def optimize_or_load(split, signals, targets, vol_forecast, dates, frequency, metric_fn):
    """Select parameter on validation, then reuse it on test."""
    spec = _read_spec()
    ...
    if split == "val":
        ...
        def objective(trial):
            ...
            score, _, _ = _evaluate(
                value, spec, signals, targets, vol_forecast, dates, frequency, metric_fn
            )
    ...
    _, weights, returns = _evaluate(
        best_value, spec, signals, targets, vol_forecast, dates, frequency, metric_fn
    )
